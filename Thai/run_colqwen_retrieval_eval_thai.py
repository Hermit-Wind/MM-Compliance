#!/usr/bin/env python3
"""
ColQwen2.5 Retrieval Evaluation for Thai RegCom dataset

Evaluates retrieval performance on Thai dataset by industry.
"""
...
QUERY_DIR = BASE_DIR / "queries_thai_by_industry"
OUT_DIR = BASE_DIR / "results_colqwen_eval_thai"
...
def get_doc_prefix_from_image(img: str):
    ...
    return img.rsplit("_p", 1)[0].lower().replace(' ', '_')
...
def load_pages_and_queries(industry: str):
    ...
    # Find query file for this industry
    industry_dir = QUERY_DIR / industry
    qfile = industry_dir / f"queries_{industry}.json"
...
def encode_images_with_cache(images, processor, model):
    ...
    cache_file = CACHE_DIR / "cache_colqwen_image_emb_thai.pt"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="ColQwen2.5 Retrieval Evaluation (Thai)")
    parser.add_argument("--industry", required=True, 
                       choices=["banking", "commerce", "energy"],
    ...
    print("\n" + "=" * 90)
    print(" " * 20 + "ColQwen2.5 Retrieval Evaluation System (Thai)")
    print("=" * 90)
