#!/usr/bin/env python3
"""
Aggregate evaluation results by metric across companies - Japanese version with industry support (NEW FORMAT)

This script reads Japanese result files (organized by industry) and creates Excel tables
...
def aggregate_results_by_metric(input_dir: Path, output_dir: Path, industries: list = None):
    ...
    """
    print(f"\n{'='*80}")
    print(f"AGGREGATING JAPANESE RESULTS BY METRIC (NEW FORMAT)")
    print(f"{'='*80}")
    ...
    for industry_dir in sorted(industry_dirs):
        ...
        data_format = data.get('data_format', 'new (PDF_Page)')
...
def main():
    parser = argparse.ArgumentParser(
        description="Aggregate Japanese evaluation results by metric across companies and industries (NEW FORMAT)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process all industries (NEW FORMAT)
  python aggregate_by_metric_japanese_new.py
  
  # Process specific industries only
  python aggregate_by_metric_japanese_new.py --industries automotive energy
  
  # Process with custom input directory
  python aggregate_by_metric_japanese_new.py --input results_colqwen_eval_japanese_new
  
  # Process with custom output directory
  python aggregate_by_metric_japanese_new.py --output analysis_japanese_new
...
Directory structure:
  Input:  results_colqwen_eval_japanese_new/
    ...
    parser.add_argument(
        '--output',
        default='metric_analysis_japanese_new',  # NEW: output to new directory
        help='Output directory for Excel files (default: metric_analysis_japanese_new)'
    )
