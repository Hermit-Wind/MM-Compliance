#!/usr/bin/env python3
"""
Split evaluation results by company (CID) - Japanese version with industry support (NEW FORMAT)

This script reads Japanese evaluation result JSON files (organized by industry) 
...
Uses NEW evaluation results from query dataset built with PDF_Page field.

Directory structure:
  Input:  results_colqwen_eval_japanese_new/
          ├── automotive/results_automotive.json
          ├── energy/results_energy.json
          └── trading_companies/results_trading_companies.json
  
  Output: results_by_company_japanese_new/
          ├── automotive/
          │   ├── results_honda_motor.json
          │   ├── results_nissan_motor.json
          │   └── results_mazda_motor.json
          ├── energy/
          │   ├── results_tokyo_gas.json
          │   └── results_toho_gas.json
          └── trading_companies/
              ├── results_itochu_corp.json
              └── results_marubeni_corp.json
"""
...
def split_results_by_company(input_file: Path, output_dir: Path, industry: str):
    ...
    print(f"  Language: {data.get('language', 'japanese')}")
    print(f"  Data format: {data.get('data_format', 'new (PDF_Page)')}")
    ...
    for company, results in company_results.items():
        ...
        company_data = {
            ...
            "model": data['model'],
            "data_format": data.get('data_format', 'new (PDF_Page)'),  # NEW: include data format
...
def process_all_industries(input_dir: Path, output_dir: Path):
    ...
    print(f"\n{'='*80}")
    print(f"SPLITTING JAPANESE EVALUATION RESULTS BY COMPANY (NEW FORMAT)")
    print(f"{'='*80}")
    print(f"Input directory: {input_dir}")
    print(f"Output directory: {output_dir}")
    print(f"Found {len(industry_dirs)} industry directories")
    print(f"\n[INFO] Using NEW evaluation results with PDF_Page field")
    print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Split Japanese evaluation results by company (CID) with industry organization (NEW FORMAT)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process all industries from default directory (NEW FORMAT)
  python split_results_by_company_japanese_new.py
  
  # Specify custom input directory
  python split_results_by_company_japanese_new.py --input results_colqwen_eval_japanese_new
  
  # Specify custom output directory
  python split_results_by_company_japanese_new.py --output results_by_company_japanese_new
  
  # Both custom directories
  python split_results_by_company_japanese_new.py --input my_results --output my_output
        """
    )
    ...
    parser.add_argument(
        '--input',
        type=str,
        default='results_colqwen_eval_japanese_new',  # NEW: use new results directory
        help='Input base directory containing industry subdirectories (default: results_colqwen_eval_japanese_new)'
    )
    
    parser.add_argument(
        '--output',
        type=str,
        default='results_by_company_japanese_new',  # NEW: output to new directory
        help='Output base directory (default: results_by_company_japanese_new)'
    )
