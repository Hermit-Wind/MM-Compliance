# run_colqwen_retrieval_eval_japanese_new.py
"""
ColQwen2.5 Retrieval Evaluation by Industry (Japanese - NEW FORMAT)
...
# ================================
# PATH SETTINGS (UPDATED FOR NEW DATA)
# ================================
BASE_DIR = Path(__file__).resolve().parent
IMAGES_DIR = BASE_DIR / "images"
QUERIES_DIR = BASE_DIR / "queries_japanese_new_by_industry"  # NEW: use new query directory
OUT_BASE_DIR = BASE_DIR / "results_colqwen_eval_japanese_new"  # NEW: output to new directory
...
# Print Functions
# =====================================================
def print_evaluation_results(results, industry):
    """Format and print evaluation results"""
    summary = results['summary']
    
    print("\n" + "=" * 90)
    print(f" {industry.upper()} - RETRIEVAL EVALUATION RESULTS (NEW DATA)".center(90))
    print("=" * 90)
...
def print_per_query_analysis(results, queries, doc_names, industry, top_n=3):
    """Print detailed per-query analysis"""
    per_query = results['per_query']
    
    print("\n" + "=" * 90)
    print(f"{industry.upper()} - PER-QUERY ANALYSIS (First {top_n} Queries)".center(90))
    print("=" * 90)
    ...
    for i in range(min(top_n, len(per_query))):
        ...
        print(f"\n📝 [Query {i}]")
        print(f"   CID: {q['CID']}")
        print(f"   Industry: {q.get('industry', 'N/A')}")
        print(f"   Code: {q.get('code', 'N/A')}")
        print(f"   Topic: {q.get('topic', 'N/A')[:60]}...")
        print(f"   Metric: {q['metric'][:60]}...")
        print(f"   Filter Page: {q.get('filter_page', 'N/A')}")
...
def validate_filtering(scores, queries, doc_names, industry, num_samples=3):
    """Validate document boundary filtering"""
    print(f"\n{'='*90}")
    print(f"{industry.upper()} - FILTERING VALIDATION (First {num_samples} Queries)".center(90))
    print(f"{'='*90}")
...
def get_doc_prefix_from_cid(cid: str):
    """
    Get document prefix from CID (company name)
    Japanese CID format: "Honda Motor", "Nissan Motor", etc.
    Keep original format (with spaces) for matching with image filenames
    """
    return cid.lower()
...
def get_doc_prefix_from_image(img: str):
    """
    Extract document prefix from image filename
    Japanese images: "Honda Motor_p001.png", "Hokkaido Gas_p001.png"
    Split by "_p" and take the part before page number
    """
...
# Load data
# =====================================================
def load_pages_and_queries(industry):
    print(f"\n[INFO] Loading {industry} dataset (NEW FORMAT)")
    ...
    # Load industry-specific queries from NEW directory
    industry_dir = QUERIES_DIR / industry
    ...
    print(f"[INFO] Loaded {len(queries)} queries for {industry}")
...
def encode_images_with_cache(images, processor, model):
    ...
    cache_file = CACHE_DIR / "cache_colqwen_image_emb_japanese.pt"
...
def compute_scores_dynamic(query_embeddings_list, doc_embeddings_list, processor):
    ...
    for qi in tqdm(range(num_queries), desc="Computing query scores"):
        query_emb = query_embeddings_list[qi]
...
# =====================================================
# Process single industry
# =====================================================
def process_industry(industry, model, processor, emb_docs, doc_names):
    """Process evaluation for a single industry"""
    
    print("\n" + "=" * 90)
    print(f"PROCESSING INDUSTRY: {industry.upper()} (NEW DATA)".center(90))
    print("=" * 90)
    
    # Create output directory for this industry
    out_dir = OUT_BASE_DIR / industry
    out_dir.mkdir(parents=True, exist_ok=True)
    
    # Load industry-specific queries
    _, _, queries, gt_indices = load_pages_and_queries(industry)
    ...
    # Validate filtering
    validate_filtering(scores, queries, doc_names, industry, num_samples=3)
    ...
    # Print results
    print_evaluation_results(eval_results, industry)
    print_per_query_analysis(eval_results, queries, doc_names, industry, top_n=3)
    ...
    for qi, q in enumerate(queries):
        ...
        detailed.append({
            "CID": q["CID"],
            "industry": q.get("industry", industry),
            "code": q.get("code", ""),
            "topic": q.get("topic", ""),
            "metric": q["metric"],
            "query_text": q["query_text"],
            "filter_page": q.get("filter_page", "N/A"),
    ...
    out = {
        ...
        "industry": industry,
        "model": MODEL_NAME,
        "data_format": "new (PDF_Page)",  # NEW: indicate data format
    ...
    out_file = out_dir / f"results_{industry}.json"
    out_file.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
    
    print(f"\n✅ {industry.upper()} DONE! Results saved to: {out_file}")
...
def main():
    parser = argparse.ArgumentParser(
        description="ColQwen2.5 Retrieval Evaluation by Industry (Japanese - NEW FORMAT)",
    ...
    print("\n" + "=" * 90)
    print("ColQwen2.5 Retrieval Evaluation System (Japanese - NEW FORMAT)".center(90))
    print("=" * 90)
    print("\n[INFO] Using NEW query dataset with PDF_Page field")
    
    # Find available industries
    if not QUERIES_DIR.exists():
        raise FileNotFoundError(f"Queries directory not found: {QUERIES_DIR}")
    
    industry_dirs = [d for d in QUERIES_DIR.iterdir() if d.is_dir()]
    available_industries = [d.name for d in industry_dirs]
    
    if not available_industries:
        raise FileNotFoundError(f"No industry directories found in {QUERIES_DIR}")
    
    # Filter industries if specified
    if args.industries:
        industries_to_process = [i for i in args.industries if i in available_industries]
        if not industries_to_process:
            print(f"❌ No valid industries specified. Available: {available_industries}")
            return
    else:
        industries_to_process = available_industries
    
    print(f"\n[INFO] Industries to process: {', '.join(industries_to_process)}")
    print(f"[INFO] Total: {len(industries_to_process)} industries")
    
    # Load images once (shared across all industries)
    print(f"\n[INFO] Loading all page images...")
    img_paths = sorted(IMAGES_DIR.glob("*.png"))
    doc_names = [p.name for p in img_paths]
    images = [Image.open(p).convert("RGB") for p in tqdm(img_paths, desc="Loading images")]
    print(f"[INFO] Loaded {len(images)} page images")
    ...
    # Encode images once (shared across all industries)
    ...
    for industry in industries_to_process:
        try:
            summary = process_industry(industry, model, processor, emb_docs, doc_names)
            all_summaries[industry] = summary
        except Exception as e:
            print(f"\n❌ Error processing {industry}: {e}")
            continue
    
    # Print final summary
    print("\n" + "=" * 90)
    print("FINAL SUMMARY - ALL INDUSTRIES (NEW DATA)".center(90))
    print("=" * 90)
    
    for industry in sorted(all_summaries.keys()):
        summary = all_summaries[industry]
        print(f"\n{industry.upper()}:")
        print(f"   Queries: {summary['num_queries']}")
        print(f"   Recall@10: {summary['Recall@10']*100:.2f}%")
        print(f"   nDCG@10: {summary['nDCG@10']*100:.2f}%")
    
    print("\n" + "=" * 90)
    print(f"✅ ALL DONE! Results saved to: {OUT_BASE_DIR}")
    print("=" * 90 + "\n")
